...
*Participant breakdown of low level and high level interactions. Low level interactions are represented as letters: clicks (C), commands (K), and mouse movements (M). Exhaustion calculated by counting each interaction (K = 1, KK = 2, etc.) and disruption calculated by counting each switch from mouse to keyboard (MK = 1, KMK = 2, etc.). High level interactions are represented as regular expressions composed of the low level interactions used to accomplish command execution, copy/paste, navigation, and tab completion.*
